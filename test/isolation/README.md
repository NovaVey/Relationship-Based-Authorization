...
under their new names, plus two more that joined later: one that split off
one of them for a reason specific to this project's own fast/integration
test split, and one that proves a real concurrency property for a later
feature using this same suite's own established "real database, real race"
discipline:
...
| [`relation-index-concurrent-rebuild.integration.test.ts`](./relation-index-concurrent-rebuild.integration.test.ts) | (new, D-163 — the Leopard index)                                        | A real concurrent-rebuild race against real Postgres: no torn/partial-generation reads while `rebuildRelationMembershipIndex` runs concurrently with checks, the SAVEPOINT fix's own permanent regression guard (a poisoned transaction from a failed advisory-lock attempt must never break the "falls through unconditionally" guarantee), and a measured `TRUNCATE`-lock latency report. Added alongside, not instead of, the other files here — the same "real database, real race" discipline this suite was already built on. |

Every test in the first four files **was** an `it.todo(...)` placeholder
when this suite was first written — the specification Phase 1 through
Phase 5 of the build spec had to satisfy, written before the code it tests
existed, exactly per `.claude/commands/build-authz-service.md` §14's
delegation rules and the `test-author` subagent's own discipline: tests are
derived from the spec, not from the implementation, and are written first.
(The fifth file, added much later for D-163, follows the same
test-before-trusted discipline but wasn't part of that original Phase 1-5
spec.) None remain `.todo()`
