...
| [`exclusion-subtract-unprovable-cut.integration.test.ts`](./exclusion-subtract-unprovable-cut.integration.test.ts) | **A/B** — closes D-158's own named follow-up: a _permanent_, general-shape property (not the fixture-specific D-158 regression tests) asserting an exclusion's `subtract` branch that hits an unprovable cut (a genuine tuple-data cycle, or a chain deeper than the check's own depth budget) must never let the exclusion grant, swept across many randomly-generated schema/tuple-graph instances per resolver mechanism — mechanism 1 (TS-level recursion) and mechanism 2 (SQL relation-membership). See `docs/DECISIONS.md` D-159/D-160. Also carries a Leopard-index Candidate F sweep (D-163) reusing this file's own mechanism-2 fixtures.   | Yes             |
| [`relation-index-soundness.integration.test.ts`](./relation-index-soundness.integration.test.ts)                   | **Leopard index Candidates A/B** (D-163) — a `relationIndex: 'warm'` sweep across many randomly-generated fixtures (`generateFixture`), asserting the index never produces an `allowed` verdict the live, unmodified CTE wouldn't also produce — fail-checked by removing the lookup's own subject-match predicate (immediate red on a cross-subject witness).                                                                                                                                                                                                                                                                                        | Yes             |
