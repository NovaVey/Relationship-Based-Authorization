...
- **Non-monotone** (the schema, as reachable from the goal, uses
  ...
  `docs/DECISIONS.md` D-150, `z3-solver`) tries next — a genuine,
...
- Explicitly out of scope for this tool entirely (build spec §13): no
  ...
  D-150, not a quiet reversal of that line.)
- **The SMT tier's own real, disclosed boundary: recursion.** A goal
  ...
  originally proposed as this tier's own live proof. See D-150 for the
