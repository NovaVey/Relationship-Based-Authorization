...
(*                                                                         *)
(* ENVIRONMENT CONSTRAINT, ORIGINAL — AND ITS LATER RESOLUTION: at the time *)
(* this spec was first written, the real TLC model checker                *)
(* (`tla2tools.jar`, ships only via GitHub Releases on `tlaplus/tlaplus`) *)
(* could not be fetched — generic GitHub access was confirmed 403/blocked *)
(* in that sandbox. `npm` surfaced one real, disclosed stand-in:          *)
(* `tla-checker` (npm, `tla-rs` on GitHub) — a ~1-month-old,               *)
(* single-maintainer, third-party TLA+ model checker in Rust/WASM,        *)
(* explicitly self-described as "a lightweight alternative to the         *)
(* official TLC ... for specs that fit its supported subset," used with   *)
(* that caveat disclosed at every mention. On 2026-08-29, a later session *)
(* re-checked GitHub Releases access and found it working — the real,     *)
(* official TLC (v2.19) was run against this exact spec and confirmed the *)
(* identical result `tla-checker` had already reported: 103,052 distinct  *)
(* states, zero violations, on the real `Spec`; a genuine counterexample  *)
(* at a comparable state count on `SpecWeakened`. Full transcripts, both  *)
(* runs, in `docs/TLA-SPEC-NOTES.md`'s "Update, 2026-08-29" section.       *)
(***************************************************************************)
