...
*that* implementation, fail-check it live, then ship — the same sequence
...
over. D-158's deferred piece was a *residual risk in an already-shipped
safety property* — a second mechanism sharing one resolver's exact bug
...
write_log` — a new, separately-defined query sharing only the anchoring
   *discipline* with `assertTokenObservedOnSnapshot`, not its literal SQL
...
identity)` dedup, generalizing this project's own existing D-092
   ...
   once in the CTE's raw output, which is exactly why the *online* path's
...
3. **The single most consequential correction the adversarial-review pass
   ...
   **The second correction: more than one candidate row can legitimately
   ...
   fundamental one — two genuinely *different* frontier nodes under the
   ...
   `order by ... array_length(via_path, 1) asc` picks the *shortest* real
...
'warm'`** (new, default `'off'` — the harness's default behavior is
  ...
  **Critical precondition, found by adversarial review and stated once
  ...
  comparison arm only means anything for *pinned* checks.** Phase A's
  ...
  catch, recurring here one level up (a vacuous *mode*, not a vacuous
  *assertion* within an already-exercised mode).
  ...
  **The fix:** `checkAllQueries` gains a new parameter, `pinToken?:
  ...
  pool — the same query *text* `resolver.ts`'s private `ANCHOR_QUERY_TEXT`
...
| (new, nightly-scale, mirroring the existing PR-vs-nightly split)                     | **New**: a `relationIndex: 'warm'` full three-way run, many seeds, asserting `indexQueriesHit > 0` (the non-vacuity gate above) alongside `indexFalseGrantCount === 0`. **Scoped honestly, not oversold: this exercises Candidates A/F/G against whatever shapes the reused generator fixtures already construct at standard scale** — the self-referential group (D-021), the deep hierarchy chain (D-070), and, for Candidate F specifically, the exclusion-cutoff deep chain (D-161) reused via the dedicated row below. It is not, on its own, a claim of exhaustive coverage of every possible depth/expiry/collision interaction "at scale" — the dedicated new files further down (the DST regression guard and the real-Postgres integration test) are what actually target Candidates C/F/G's specific decision boundaries directly; this row's job is breadth across many random schema shapes, not depth on any one of them.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
transaction's end for *any* reason — a normal `COMMIT`/`ROLLBACK`, or the
...
(`LEOPARD_INDEX_REFRESH_INTERVAL_MS`) exists for a deployment that
doesn't want to stand up a separate cron at all; both triggers call the
same function, guarded by the same advisory lock, so layering both is
safe, only possibly redundant.
...
| `authz serve`                                    | No background timer is ever created (`LEOPARD_INDEX_REFRESH_INTERVAL_MS` is only consulted when `LEOPARD_INDEX_ENABLED === 'true'`).                                                                                                           |
...
- **This inherits, and does not attempt to tighten, `atToken`'s own
  floor-not-ceiling looseness — the *property* is identical, the typical
  *magnitude* is not, and saying only "exactly as a live pinned check
  ...
  Both are the same *kind* of looseness (a floor, never a ceiling; never a
  ...
  *property*; it materially widens the *typical gap*, and says so.
