...
`test/thirdparty-survey.test.ts` (`tools/schema-verifier/test/`) now
...
**The six same-shape entries D-131 couldn't reach have since closed
too.** `docs/DECISIONS.md`'s entry adding `NeverRelationConstraint` built
exactly the schema-level primitive this section's own earlier text named
as the real fix — "this relation can never be satisfied via any object,
anywhere" — and closes all six: `openfga-github`, `spicedb-superuser`,
`spicedb-docs-style-sharing`, `openfga-gdrive`, `openfga-slack`,
`spicedb-github`. Each gained one or more `never <namespace>#<relation>
(<var>)` lines naming the exact relation(s) their own second,
structurally different escape (a userset-subject or recursive path, or —
for `spicedb-superuser` and `openfga-gdrive` — a completely separate
top-level union term) ran through; see each row below for the specific
lines. **Current tally: 2 VIOLATED, 10 HOLDS.** The remaining two
(`openfga-expenses`'s self-referential manager loop,
`spicedb-userdefined-roles`'s own distinct unconstrained-second-tuple
escape, both described above) are structurally different shapes neither
primitive was ever designed to reach. `test/thirdparty-survey.test.ts`
pins these six new verdicts too.

## Results

| Schema                             | Source                                                                                                                                 | Invariant tested                                                                                                                                                       | Verdict                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Witness (if violated)                                                                                                                                                                                                                                                                                 |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openfga-github`                   | [openfga/sample-stores: github](https://github.com/openfga/sample-stores/blob/main/stores/github/model.fga)                            | `plain_org_member_never_gets_repo_admin` — an org member (not owner) never gets repo admin without an explicit repo-level grant                                        | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by `docs/DECISIONS.md`'s `NeverRelationConstraint` entry: `never repo#admin_direct(s)` and `never organization#repo_admin(s)`, ruling out both the direct repo-admin grant and the independent org-admin escape the invariant meant to probe)                                                                                                                                                                                                                                                              | —                                                                                                                                                                                                                                                                                                     |
...
| `spicedb-superuser`                | [authzed/examples: superuser](https://github.com/authzed/examples/blob/main/schemas/superuser/schema-and-data.yaml)                    | `document_admin_requires_ownership_chain` — document admin requires an actual ownership chain                                                                          | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by `never document#owner_user(u)` and `never platform#administrator(u)`, ruling out the direct owner grant and the unconstrained superuser-platform escape. The source's _own_ README and fixture assertion (`document:lasers#admin@user:drevil`, reached only via the platform's `super_admin`) confirm a site-wide superuser backdoor is the schema's deliberate design — this invariant's own `u` is never given any standing on that platform at all, so the backdoor itself is untouched, not hidden) | —                                                                                                                                                                                                                                                                                                     |
| `spicedb-docs-style-sharing`       | [authzed/examples: docs-style-sharing](https://github.com/authzed/examples/blob/main/schemas/docs-style-sharing/schema-and-data.yaml)  | `sibling_group_member_cannot_view_other_group_document` — a member of one group can't view a document shared only with a sibling group                                 | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by `never document#viewer(a)` and `never group_with_parent#member(a)`, ruling out both the direct viewer grant and the recursive group-membership escape, with `a`'s own real membership in `analysis` exempted rather than blocked)                                                                                                                                                                                                                                                                       | —                                                                                                                                                                                                                                                                                                     |
| `openfga-gdrive`                   | [openfga/sample-stores: gdrive](https://github.com/openfga/sample-stores/blob/main/stores/gdrive/model.fga)                            | `sibling_folder_viewer_cannot_read_document` — folder-level access to a sibling folder doesn't grant read on a document filed elsewhere                                | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by four `never` lines (`doc#viewer`, `doc#owner`, `folder#owner`, `folder#viewer_direct`), ruling out the direct viewer grant, the sibling owner union term, and the recursive ancestor-folder escape)                                                                                                                                                                                                                                                                                                     | —                                                                                                                                                                                                                                                                                                     |
| `openfga-slack`                    | [openfga/sample-stores: slack](https://github.com/openfga/sample-stores/blob/main/stores/slack/model.fga)                              | `workspace_guest_never_becomes_channel_writer` — a workspace guest never becomes a channel writer                                                                      | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by `never channel#writer(u)`, ruling out the direct writer grant unrelated to the `guest` relation)                                                                                                                                                                                                                                                                                                                                                                                                        | —                                                                                                                                                                                                                                                                                                     |
| `spicedb-github`                   | [authzed/examples: github](https://github.com/authzed/examples/blob/main/schemas/github/schema-and-data.yaml)                          | `org_member_never_gets_repo_admin_without_role` — same question as `openfga-github`, checked against SpiceDB's own independently-authored model of the same domain     | **HOLDS** (exact, monotone; empirically confirmed clean across 25 sampled tuple sets — closed by `never repository#admin(u)`, the same shape as the OpenFGA entry above despite a structurally different schema)                                                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                                                                                                                                                                                                                     |
...
**2 VIOLATED, 10 HOLDS, 0 UNKNOWN, 0 tool errors.** (Originally 9 VIOLATED,
...
bringing it to 8 VIOLATED/4 HOLDS; `openfga-github`, `spicedb-superuser`,
`spicedb-docs-style-sharing`, `openfga-gdrive`, `openfga-slack`, and
`spicedb-github` then all moved to `HOLDS` per `docs/DECISIONS.md`'s
`NeverRelationConstraint` entry, bringing it to the current 2 VIOLATED/10
HOLDS — see "The recurring finding" above for the full account of every
correction.) Every `VIOLATED` entry is self-validated per rule 2 above.
