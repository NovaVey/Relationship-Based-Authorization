...
`authz serve` exposes the same operations over HTTP, plus three bulk
reverse-lookup/scope operations with no CLI command of their own:
...
| `POST`   | `/scope`          | `ADMIN_API_KEY` or `READONLY_API_KEY` | 20/min     | For up to 50 caller-supplied (namespace, permission) targets, does subject hold at least one grant? Bounds a delegation credential's own scope; one target's own failure never aborts the rest (D-186) |
...
src/
  ...
  audit/       expand(), listObjects()/listUsers() (listObjects reverse-lookup-accelerated, D-175), scope.ts (queryScope, the scope-bounding query for delegation-credential minting, D-186), privesc.ts (privilege-escalation scanner, D-152), the hash-chained checks audit trail every real check is logged to (tamper-evidence, D-148), and anchor.ts (the out-of-band, append-only tip anchor, D-155)
