...
## Five independent features, built in parallel, each in its own isolated worktree (D-145 through D-149)

**Owner:** the main agent, dispatching five independent agents in parallel (isolated git worktrees, no file overlap) for the "start here"/"high-value" tier of the feature-ideation list above, after the scope decision (D-144) and after checking two hard blockers first — the SMT decision-procedure idea needs `z3-solver`, a new dependency not added without asking; TLA+'s model checker isn't available in this sandbox — both flagged rather than faked.

1. **`PG_POOL_MAX` startup guard (D-145)** — D-140's own weaker, still-open option ("document the invariant explicitly, or enforce it at startup"), picked up now that D-143 closed the actual deadlock. `authz doctor` warns (soft, not a hard failure) when `MAX_CONCURRENCY >= PG_POOL_MAX`, naming both numbers. Verified live against real Postgres, not just mocks.
2. **Schema-parser crash-safety fuzzing (D-146)** — closes the class of bug D-132 found manually. Found a real, previously-undiscovered bug live: a flat, unparenthesized exclusion chain has no depth ceiling (exclusion isn't associative, so it's never flattened like `|`/`&`), letting a ~5,000-term chain throw a raw `RangeError`. Fixed by charging exclusion links against the same nesting-depth ceiling `(` already uses. Fail-checked twice — once on the real bug, once on the fuzz test's own checker helper, which had the identical flaw.
3. **General exclusion anti-monotonicity sweep (D-147)** — generalizes Property 5 (D-140) beyond its one hand-verified shape, using a new `findFlippableExclusion` extension of `classifyMonotone`'s own AST walk. Independently re-fail-checked by this entry's own author (not just the design agent) by weakening the resolver's real exclusion evaluation live.
4. **Hash-chained checks audit log (D-148)** — tamper-evidence for the audit trail this project's whole "re-derivable evidence" pitch depends on. `insertCheckRow` previously had zero locking or transaction at all; now opens its own advisory-locked transaction and chains each row's hash to the true previous row. Live fail-check: tamper with one committed row via raw SQL, confirm `authz audit verify` names that exact row.
5. **`authz schema diff`/`authz schema rollback` (D-149)** — catches a publish that would silently revoke access before it ships, reusing `classifyMonotone`-style structural reasoning; rollback republishes a historical version's already-stored source. Fail-checked both failure directions (a forced false-positive, a disabled true-positive rule) and proven end to end: a real narrowing publish is warned about, actually revokes a real grant, and rollback actually restores it.

**A real, serious problem hit during this batch, not glossed over: two agents' own structured output came back with corrupted file content — a literal `"PLACEHOLDER"` string for three files, and a natural-language description ("see the file on disk") in place of two others — despite each agent's own summary and verification claims describing genuinely real, thorough work.** Caught by independently re-running `npx tsc --noEmit` across the combined result before trusting any of it, not by assuming five green individual reports meant a green whole. Recovered the three `PLACEHOLDER` files directly from that agent's still-live worktree before it was cleaned up; the two description-only files had already lost their source worktree, so this entry's own author manually reconstructed both fixes by reading the new `insertCheckRow`'s real transaction shape directly and rebuilding each mock to match — confirmed via live LOCALVERIFY runs, not assumed correct because they compiled. This is exactly the kind of gap this project's own discipline exists to catch before it ships, not after.

**A second, independent problem, from a different root cause: two agents (D-148, D-149) each independently modified the same shared file, `src/cli/index.ts`, in their own isolated worktrees — wiring `authz audit verify` and `authz schema diff`/`rollback` respectively.** Applying both agents' file sets sequentially meant the second one applied silently overwrote the first's changes to that shared file — `authz audit verify` briefly vanished from the CLI entirely, with nothing about it looking wrong until checked directly (`grep -n "audit" src/cli/index.ts` came back empty). Caught the same way: never trusting "no errors reported" as proof of correctness, checking the actual merged file's real content against what each agent's own summary claimed it had done. Fixed by hand-merging both agents' independent `index.ts` additions into one file, then re-confirmed with a real, un-mocked CLI invocation of both commands end to end (`authz audit verify` against real Postgres — `160/160 rows verified, chain intact`; `authz schema --help` listing both `diff` and `rollback`), not just a passing test suite.

**Verification:** `npx tsc --noEmit`, `npx eslint .`, `npx prettier --check .`, `npm run build` all clean across the full combined change set. Root fast suite: 59 files, 882 tests (up from 56/811). Every real-Postgres integration test touched or added by this batch (5 files) independently re-run live via this repo's own established LOCALVERIFY accommodation by this entry's own author — not merely trusted from agent reports — each restored to its exact committed, container-based form and confirmed byte-clean afterward.

Full account of each: `docs/DECISIONS.md` D-145 through D-149.

## Expiring/time-boxed relation tuples — D-144's own deferred implementation, built (D-150)

**Owner:** the main agent, dispatching four independent, fully disjoint-file agents in isolated worktrees (storage layer; the reference resolver; cache/audit cache-safety wiring; CLI+API wiring) for the pieces that could be pinned to an exact, unambiguous interface contract up front, while personally building the one genuinely coupled piece — the production resolver plus its DST-fake fidelity twin — directly, the same "owner: the main agent" precedent D-143 set for tightly-coupled, correctness-critical work.

Asked to build D-144's own narrow, closed-form time-window caveat now that the scope decision authorizing it was settled. Shipped: a nullable `relation_tuples.expires_at`; independent expiry logic in both resolvers (D-022's isolation preserved); the identical fix applied to `expand()`'s own separate query, found by checking directly rather than assuming the resolver fix alone was enough; a `touchedExpiringTuple` flag and a cache-safety fix closing the exact risk D-144 flagged (a cached `ALLOW` could otherwise outlive a tuple's real expiry, since expiring produces no write for the cache's own invalidation machinery to react to); a new `docs/CONSISTENCY.md` section stating the guarantee plainly; full DST-fake fidelity including a new controllable fake clock; CLI/API wiring; and the two live fail-checks D-144 itself named as prerequisites — a new DST fault proving snapshot isolation composes correctly with an expiry crossing mid-transaction, and a real-Postgres integration test proving the same story end to end.

**Parallelized correctly this time, directly applying the previous batch's own two lessons.** Every agent got a fully pinned interface contract (the exact final shape of every new type/field) rather than being left to design any shared piece independently, and — critically — no two agents ever touched the same file, closing off the shared-file-collision failure mode D-148/D-149 hit entirely by construction rather than by hoping to catch it again after the fact. Every agent's own returned summary was still independently verified by reading its real files directly from its own live worktree and re-running the actual toolchain on the assembled result — the same discipline that caught the previous batch's structured-output corruption — rather than trusted at face value; this time, every agent's real diff matched its own summary exactly, no corruption found.

**A second real, disclosed cross-cutting gap, distinct in kind from the previous batch's shared-file collision: DST's fake store matches every query by exact SQL text, so changing a real query's own SQL text (adding a column, adding a filter) silently invalidates whatever shape was registered for the old text.** The storage-layer agent found this itself — by running the full test suite beyond what its own narrow task asked for — and disclosed exactly what broke (126 failing tests, one root cause) rather than attempting an unauthorized fix outside its assigned files. The main agent reconciled every affected registered shape in `shapes.ts` against the real, final SQL text once all four pieces had merged, plus one more site the same pass caught: a DST test hand-constructing a raw insert statement outside `writeTuple` entirely, which needed the identical update. Full suite re-run clean afterward.

**Four separate live fail-checks, targeting four distinct mechanisms**, not just "the tests pass": the new DST fault's own snapshot-anchoring claim (bypassing the anchor made it fail for the predicted reason); the same check/expand-parity claim against real Postgres (removing the resolver's own liveness filter broke both real-Postgres integration tests); and the cache-safety fix specifically, isolated from the resolver's own correctness (removing just the cache guard broke only the cache-safety integration test, not the parity one). Every real-Postgres integration test in this batch — including the storage-layer agent's own new one, which that agent had disclosed it could not execute itself (no Docker in its own environment either) — was independently re-run live by the main agent via this repo's own established LOCALVERIFY accommodation (a local Postgres cluster started for the purpose, standing in for the sandbox's unavailable Docker daemon), each restored to its exact committed, container-based form and confirmed byte-clean afterward.

**One design question decided deliberately and disclosed, not silently skipped:** `listObjects`'s own coarse candidate-gathering query is intentionally left unfiltered by expiry — every candidate it returns is independently re-verified by a real `productionCheck` before ever appearing in a result, so over-including an expired candidate costs one extra, correctly-denied check and can never produce a wrong final answer; filtering it too would be speculative complexity with no correctness benefit.
...
**A real finding from a class of tool this project's own local verification never runs: GitHub's CodeQL, surfaced only after opening the pull request.** `js/insufficient-password-hash` flagged `hashApiKey`'s plain SHA-256 digest — a real rule, aimed at a different threat model than this one (low-entropy, human-chosen secrets, where a fast hash makes brute-forcing a leaked hash cheap). `hashApiKey` only ever hashes `generateRawApiKey`'s own 256-bit CSPRNG output or a lookup candidate compared against rows that all came from that same source, making brute force infeasible regardless of hash speed — the same bet GitHub's and Stripe's own API-key systems make. Verified as a genuine false positive for this input shape, not blindly accepted. Two inline `codeql[...]` suppression attempts didn't clear the alert on either commit's own re-scan (this repo's CodeQL setup doesn't appear to honor them); converged by substance instead — `hashApiKey` now derives its digest via `scryptSync` (no new dependency), with deliberately modest cost parameters since it runs on every gated request, not once per login. Unit and integration test fixtures updated to match; re-verified live via LOCALVERIFY, all 5 tests passing against a real database.
...
**A real numbering collision, caught during reconciliation, not shipped:** this entry's own worktree and the CHC-tier worktree (D-153) were dispatched from the same base commit, neither able to see the other's own choice of decision number — both independently wrote their own `docs/DECISIONS.md` addition as `D-153`. Caught by the parent session diffing both worktrees' doc changes directly before committing either; this entry's own heading and cross-references were renumbered to `D-154`.
...
## A Horn-clause/CHC tier decides the schema verifier's recursive fragment, using z3's real PDR/Spacer engine (D-153)

**Owner:** a soundness-engineer agent in an isolated worktree, dispatched alongside D-154's and D-155's own agents in one parallel batch, at high effort given the genuine novelty of the work.

D-151's own non-recursive SMT tier explicitly declined on any goal whose rewrite tree cycles back to itself, naming a Horn-clause/CHC fixpoint solver as "the real v2 answer." This entry is that answer: `tools/schema-verifier/src/smt/chc-encode.ts`/`chc-witness.ts`/`chc.ts` compile a recursive schema's own rewrite rules into Horn clauses and query z3's real `Fixedpoint` (PDR/Spacer) engine directly, confirmed available on the already-approved `z3-solver` dependency by reading its own type definitions before designing anything.

Two real findings from exercising the API directly, not assumed from documentation: every namespace has to share one Z3 `Int` sort rather than a per-namespace uninterpreted sort (an uninterpreted domain makes `Fixedpoint.query()` return `unknown` even for a trivial ground fact — Spacer's model-based projection needs theory structure it doesn't have otherwise); and negation of a registered relation is confirmed non-functional in this z3-solver build's `Fixedpoint` engine, reproduced across every engine/configuration tried on the flattest possible case. Given that, `exclusion`/`notRelationEquals` are out of this tier's scope by disclosed design, not oversight — the repo's own CI-checked invariant (`banned_member_never_views_org`, exclusion-reaching) is unaffected, still decided by bounded search exactly as before.

The real live-proof target: `schema/example.authz`'s own `folder#sensitive_review` — D-151's own named "headline demonstration" that recursion made it decline on — is now decided exactly (`VIOLATED`, `proof: exact`, a real witness independently confirmed through the unmodified production engine), where it was previously `UNKNOWN`. Self-verification fail-checked live exactly like every other tier in this codebase: a 30-level recursive chain exceeding the real engine's own depth ceiling produced a wrong `VIOLATED` with the confirmation gate disabled; reverted, confirmed correct, kept as a permanent regression test.

**Verification:** `npx tsc --noEmit` clean on both root and `tools/schema-verifier` configs, `npx eslint .`, `npx prettier --check .` clean. `tools/schema-verifier` suite: 19 files, 183 tests (up from 18/170). Independently re-confirmed live by this session's own main agent via the real CLI (not just the test suite): `folder#sensitive_review` against the real schema now reports `VIOLATED, proof: exact, self-validation: confirmed`; `banned_member_never_views_org` unaffected at `HOLDS up to k = 1, proof: bounded`.

Full account: `docs/DECISIONS.md` D-153.

## An out-of-band, append-only anchor for the checks hash chain, closing D-148's own named residual risk (D-155)

**Owner:** a general-purpose agent in an isolated worktree, dispatched alongside D-153's and D-154's own agents in one parallel batch.

D-148's own hash-chained audit log detects a single tampered row, but disclosed a real, named limit: it cannot detect a privileged database user rewriting the whole chain forward, consistently, from a tampered row — recomputing every `row_hash`/`prev_hash` so the internal walk alone reports the chain intact. Closing that needs somewhere to record the chain's own tip that the same privileged user genuinely cannot reach — which rules out a new Postgres table outright (a privileged DB user could rewrite that too). `src/audit/anchor.ts` records the chain tip to a local, append-only NDJSON file instead, with `authz audit anchor` to record one and a new `authz audit verify --anchor-file` flag that re-derives what the chain hashes to at each anchored position and compares against what was recorded — reporting a distinct `ANCHOR MISMATCH`, never conflated with a plain single-row tamper. The code and the CLI both state plainly that a local file alone provides no real protection until an operator replicates it somewhere this same Postgres instance genuinely cannot reach — this ships the real mechanism, not someone else's infrastructure.

A real bug found via live verification, not assumed correct because it compiled: `readChainTip`'s own SQL aliased `chain_seq::text` and ordered by that alias, sorting lexicographically instead of numerically — silently returned `chain_seq=9` as "the tip" with `chain_seq=10` present. Fixed by dropping the redundant cast.

**Fail-checked live, the actual point of this work.** Against real Postgres: recorded a real anchor, then performed a genuine full consistent-forward rewrite of the chain via raw SQL. Confirmed, as two separately observed facts: a plain `authz audit verify` with no anchor reports the rewritten chain as fully intact (the real gap, proven to exist, not assumed); `authz audit verify --anchor-file` correctly detects and reports it. Independently reproduced by this session's own main agent afterward, end to end through the real CLI (schema publish, tuple write, 5 checks, `audit anchor`, a raw-SQL forward rewrite, then both halves of the fail-check) — identical result both times.
...
## Post-Leopard-index response to an external review — four parts

**Owner:** main agent (part 1, disclosure tightening) + soundness-engineer (parts 2a/2b, DST design + implementation) + general-purpose (parts 3/4, TLA+ spec, benchmark harness) — every part independently re-verified by the main agent before being committed.

An external review asked the four questions that actually distinguish a Leopard index from a materialized cache — watermark-gated fallback, real Zanzibar structure vs. closure table, edge-deletion handling, differential test — and named DST evolution / a formal spec / a benchmark harness as legitimate next steps. All four questions checked out against the real code; the two nuances worth surfacing were pulled higher into `docs/LEOPARD-INDEX-PROPOSAL.md`'s own Status header. The user then asked for all three next-step projects.

**DST evolution** (`docs/DST-LEOPARD-EVOLUTION-PROPOSAL.md` → `src/store/dst/*`): DST hadn't absorbed the Leopard index's async surface at all — confirmed via `grep`, not assumed. Built a writable snapshot mode, a non-blocking advisory-lock primitive, state for the two new tables (one design correction along the way: a first-draft "generation list" model was wrong, caught against the real concurrent-rebuild test's own documented findings), and a genuinely new "poisoned connection" fault class modeling the exact D-163 SAVEPOINT bug shape. Found a real bug in its own new machinery during the mandated fail-check (a one-shot latch needed to stop a poison condition re-firing on the very next statement) and a real, disclosed off-by-one in the design's own shape count. Fail-checked 4 times with real breaks and restores; the riskiest one (D8, the SAVEPOINT recovery invariant) independently reproduced a second time by the main agent. Fast suite: 75 files, 1147 tests (up from 73/1119).

**TLA+ spec** (`docs/leopard-index.tla`/`.cfg`, `docs/TLA-SPEC-NOTES.md`): formalized the watermark-publication protocol's staleness invariant, checked it clean on the real design (103,052 states) and found a real counterexample on a deliberately weakened variant (57,120 states) — the "break it and get a trace" discipline applied to a formal spec. The real TLC model checker couldn't run in this sandbox (GitHub-Releases-only, blocked by the same network policy that blocks Docker Hub) — disclosed plainly rather than faked; a real but unofficial WASM reimplementation (`tla-checker`, independently confirmed to exist) was used as a labeled, supplementary check instead.
...
## Fourth and final capability-gap item: public/wildcard subjects (`<ns>:*`), reopening D-114

**Owner:** main agent.

The biggest, riskiest item of the four, deliberately built last: it touches the schema grammar, both check resolvers, `listUsers`'s own concrete-subject-only contract, and the exclusion-soundness history (D-158 through D-161) all at once. Front-loaded a dedicated design-discovery workflow before writing any code — six parallel readers over the schema DSL, both resolvers, `listUsers`, the soundness fuzzing harness, and the store layer, followed by three independent design proposals scored against each other and synthesized into one decisive, file-by-file implementation plan — rather than designing this one inline the way items 1-3 were.

A relation now accepts `<ns>:*` as a subject type, opt-in per relation (`relation viewer: user | user:*`). The core soundness argument: both resolvers already funnel every rewrite mechanism through exactly one subject-comparison site each; the wildcard fix touches only that one site in each, so `base` and `subtract` in an exclusion can never disagree about what counts as a match — the exact symmetry that keeps a D-158-class bug from recurring, by construction rather than empirical luck. `listUsers` needed more than a type widening: its pre-existing set combinators were literal-key operations that would have silently mis-handled a wildcard inside an exclusion's own `subtract` branch (a real, distinct trap from the "flattens a stored `user:*` into one literal entry" bug this replaces) — fixed with a `MemberSet{concrete, wildcardNs}` representation whose union/intersection stay exact and closed-form, and whose subtraction refuses outright (rather than approximating) the one genuinely co-finite shape it can't represent ("everyone of a namespace except finitely many named exceptions").

Two new permanent, real-Postgres metamorphic properties are the mandatory gate this feature required before shipping (mirroring D-160/D-161's own precedent for the two prior exclusion-soundness mechanisms): one proving a wildcard-excluded subject is denied on both the production and reference engines across 30 random seeds (plus that a _different_ object's real, unrelated deep-chain uncertain-cut behavior isn't regressed by the schema merely also declaring a wildcard elsewhere), one proving `listUsers` itself gets all four wildcard-specific traps right, including the exact co-finite refusal shape. A provisional decision number picked during the design pass turned out to already be taken by the time implementation started (three other items had shipped in between) — caught and mechanically renamed across every file before committing, not left as a stale collision.
...
## Seventh item from the broader backlog: a Watch endpoint

**Owner:** main agent.

`docs/CAPABILITY-GAPS.md`'s "A Watch endpoint" section named this a real, wide-open gap with an unusually solid foundation already sitting there: `write_log` already stores exactly `(token, operation, tuple, written_at)` per change, with a genuinely monotonic token, but nothing anywhere reads that log — no SSE, no LISTEN/NOTIFY, not even a plain polling GET.

`GET /watch?since=<token>&namespace=<ns>` closes it: a live Server-Sent-Events stream of every `write_log` row from `since` (or "now," if omitted) forward. The one real design decision — DB-polling per connection, not Postgres LISTEN/NOTIFY — trades push-instant latency for something much simpler to reason about and, as a genuine side benefit, gets multi-replica fan-out entirely for free: every open connection runs its own independent poll loop against the one shared `write_log` table every replica already reads from, so no Redis and no cross-process signaling of any kind is needed, unlike the rate-limiter/flood-guard state this same codebase does need `REDIS_URL` for.

This is also the first code anywhere in the repo to read `write_log.tuple`'s actual contents (every prior caller only ever read `max(token)`) — confirmed live before writing it that a `Date` field written through `JSON.stringify` comes back out of `jsonb` as a plain string, never a `Date`, so the new `WatchTuple` type is deliberately distinct from `TupleKey` rather than a reuse of it.

Testing this needed a real methodological finding of its own: `app.inject()`'s streaming mode (`payloadAsStream: true`) resolves immediately and streams real chunks over real time, which is what makes it usable to test `/watch` at all without a listening socket — but a _simulated_ injected request never fires a real `'close'` event, even when the client-side stream is destroyed. Left alone, every test-opened `/watch` connection would loop forever in the background. Fixed by finding (confirmed via `===`) that the injected response's own `res.raw.res.req` is the exact same object the route handler's `request.raw` refers to, so manually emitting `'close'` on it from the test triggers the same cleanup a real disconnect would. A dedicated final test, using a real `app.listen()` — the one deliberate exception in this whole repo to the `app.inject()`-only convention — then proves a _real_ client's disconnect does the same thing, closing the gap the workaround itself couldn't prove.

Verified live against a real, natively-installed Postgres across two new integration test files (28 cases combined): real catch-up-then-live delivery in commit order, `?since` reconnect replaying exactly what was missed, scoped-credential namespace enforcement mirroring `/list-objects` and `/metrics` combined, `WATCH_MAX_CONNECTIONS` genuinely capping and freeing connections (including over a real socket), and a heartbeat on an idle connection. Two real, unrelated test-fixture bugs were caught and fixed along the way, not silently routed around: an object identifier using a hyphen instead of the required grammar, and a fixture namespace that needed a real published schema `writeTuple` actually requires. Real OS-level backpressure (`WATCH_MAX_BUFFERED_BYTES`) was disclosed as not exercised live — forcing genuine TCP receive-window exhaustion deterministically was judged a real flakiness risk not worth taking for a single, directly-readable conditional.

Full account: `docs/DECISIONS.md` D-174.

## Eighth and final item from the broader backlog: a reverse-lookup accelerant for `listObjects`

**Owner:** main agent.

The last remaining item on the capability-gap backlog. Flagged, before any design work began, as comparable in scope and risk to the original Leopard index build — a new soundness-critical acceleration structure, not a same-day fix — and the user confirmed: build it, with the same full rigor (design, adversarial review, then implementation) the wildcard-subjects item and the Leopard index itself both went through.

`listObjects` today enumerates candidates with a plain, unfiltered, namespace-wide scan capped at 1000 — a subject who can see one object out of 50,000 in a namespace can get silently truncated before that one object is ever reached. A first design draft proposed reusing Leopard's own floor-comparison freshness gate (`watermark >= atToken`) unchanged. A genuine three-reviewer adversarial pass found this was the wrong comparison for this feature's shape, not merely an edge case: two reviewers independently constructed the same counterexample (a write landing after the rebuild watermark but before the caller's own pin), and a third found an unrelated second way to the same silent-omission outcome (a live rebuild's `TRUNCATE` racing the read). None of this was patched around — the freshness gate was redesigned to compare against the current write-log token read live at call time instead of the caller's own floor, an empty-or-errored accelerated result became an unconditional miss, and wildcard-capable relations were excluded from acceleration entirely rather than handled inside it (closing a second real cost the reviewers found: a SQL re-implementation of the resolver's own canonical subject-match rule that could also silently erase the whole optimization's benefit).

What shipped reuses the Leopard index's own `relation_membership_index` table unchanged — no new table, one new secondary index for the reverse direction. Every candidate this feature proposes still passes through the exact same, real, unmodified per-object check `listObjects` already ran before this feature existed, which is what makes the soundness argument nearly free regardless of any of the new gates; the real risk this feature carries is completeness, not soundness, and the whole design centers on that distinction.

Implementation surfaced two real corrections to the design's own stated test plan, both disclosed rather than silently diverged from: gate ownership (which checks live in which file) had to follow this codebase's existing rule that the store layer never imports from the schema/resolver layers, splitting the five gates across two files differently than the design doc first assumed; and the metamorphic soundness sweep compares the accelerated path against the same function with acceleration off, rather than re-deriving a second independent brute-force oracle a sibling test file already owns. A live experiment against this sandbox's own real Postgres also turned up a genuinely new finding: unlike Leopard's own forward-direction lookup, this feature's own reads never sit inside a pre-existing transaction, so a reader blocked behind a concurrent rebuild's `TRUNCATE` sees the fully-populated new generation once unblocked, never a torn or empty read — a materially different, and easier, race than the one Leopard's own forward direction has to defend against.
...
## Closing the wildcard-soundness question the OpenFGA front end disclosed — one real bug found outside the schema verifier, one inside it

**Owner:** main agent.

Asked to pick up the quicker of the two remaining open items, chose the wildcard-soundness question the OpenFGA front end work had disclosed but deliberately left open: whether the schema verifier's own proof engines reason correctly about a relation that only accepts a wildcard subject, in every case, not just the one quick spot-check that earlier work happened to try. Given how open-ended that question was, it got a properly resourced investigation rather than a quick second guess — several independent readers covering each proof tier, several more building and actually running real test schemas against each one's specific reasoning, and a final pass whose only job was to try to tear down whatever the others found before any of it got trusted.

That process found two real things, not one, and they turned out to be connected. The first is a genuine bug in the core tuple store itself, nothing to do with the verifier tool at all: a relation declared to accept _only_ the wildcard form was supposed to refuse an ordinary grant to one specific named individual — that's the whole point of declaring it that way — but the store's own validation let such a grant through anyway, a real gap in a check that was supposed to be symmetric and wasn't. The second, inside the verifier's own weakest proof tier (the last-resort exhaustive fallback it only reaches once faster proof methods decline), was a genuine structural blind spot: that tier never considered the one witness a wildcard-only relation can actually be satisfied by, so a schema that really is unsafe could, at least in principle, have been reported as safe with no larger search bound ever able to fix it. What made this one tricky to catch is that the first bug was accidentally covering for the second the whole time — fixing the store bug on its own, with nothing else changed, would have quietly flipped a correctly-caught violation into a false all-clear. Demonstrated directly, not just argued: built the one schema shape that forces the fallback tier to run, and showed the verdict really does flip depending on which bug is or isn't present.
