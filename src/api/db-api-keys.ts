 ...
 * (`src/api/auth.ts`, `migration 0007_api_keys.sql`). Where the static keys
...
/** Mirrors `checkAdminAuth`/`checkReadAuth`'s own two-tier split exactly — see the `0007_api_keys.sql` migration's own `role` column comment for why there is, and will only ever be, exactly these two values. */
